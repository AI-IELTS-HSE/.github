...
2. [Разработка продукта](#Разработка-продукта)
   ...
   -  [конструирование / реализация требований, архитектуры ПО и графического интерфейса](#4-construction---конструирование-/-реализация-пунктов-1-,-2-и-3) -> TODO(shvdanil) не работает ссылка
   -  [контроль качества реализации](#5-qa---контроль-качества-реализации)
3. [Выпуск продукта](#Выпуск-продукта)
4. [Встречи и жизненный цикл спринта](#Operational:-встречи-и-жизненный-цикл-спринта)
   - [этапим](#1-Этапим-~-45-мин) -> TODO(shvdanil) не работает ссылка
   - [описываем задачи](#2-Асинхронно:-описание-задач) -> TODO(shvdanil) не работает ссылка
   - [взвешиваем задачи](#3-Асинхронно:-взвешивание-задач) -> TODO(shvdanil) не работает ссылка
   - [приоритизируем задачи](#4-Приоритизация-задач-~-30-мин) -> TODO(shvdanil) не работает ссылка
   - [планирование задач](#5-Асинхронно:-планирование-задач) -> TODO(shvdanil) не работает ссылка
5. Продажи продукта
...
Емкость спринта – сколько в целом задач влезет в спринт = 7 астрономических дней * количество человек

Квота – разделение задач спринта на категории:
- бизнес квота: реализация новых фичей, например 80% емкости спринта
- повышение качества (refactor, produce задачи), например 10% емкости спринта
- исправление багов (fix задач), например 10%
